...
function create() {
    $('button.new').on('click', function () {
        var $newLi = '<li class=""><h3 class="">New Note</h3><span class="closeLi close" aria-hidden="true">&times;</span><div class="content">empty note</div><p class="time">' + calculateDatetime() + '</p><input type="hidden" class="noteId"/></li>';
        $('.noteList').prepend($newLi);
        bindLiClick();
        bindClose();  //在这里先接触click事件再重新绑定，防止加载的li重复绑定click两次
...
function dataBinder() {
    ...
    $('textarea.content').on('keyup', function () {
        $('div.activeContent').text($(this).val());
    });
}
...
function loadData() {
    ...
    $.getJSON('../PanelsManagement/handlers/ModernNote.ashx', { id: 3520 }, function (data) {
        $.each(data, function (k, v) {
            ...
            var $Li = '<li class=""><h3 class="">' + v.NoteTitle + '</h3><span class="closeLi close" aria-hidden="true">&times;</span><div class="content">' + stripHtml(v.NoteText) + '</div><p class="time">' + date + '</p><input type="hidden" class="noteId" value=' + v.Note_ID + '></li>';
            $('.noteList').append($Li);
        });
